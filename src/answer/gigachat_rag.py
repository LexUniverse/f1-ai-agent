...
- Источник истины только эти фрагменты. Любые факты (имена, команды, места, очки, даты, итоги гонок и сезонов), которых нет в тексте фрагментов или которые нельзя однозначно вывести из них, ЗАПРЕЩЕНО добавлять из «общих знаний», памяти модели или догадок.
- Если вопрос про конкретное событие, сезон, гонку, пилота или результат, а во фрагментах нет сведений, прямо относящихся к этому запросу (например, попались только другие годы, другие этапы или обобщения без ответа на вопрос) — не придумывай и не переноси факты с «похожих» случаев. В message честно напиши, что по переданным фрагментам нельзя дать достоверный ответ. Не утверждай и не подразумевай никакой фиксированной «границы лет» покрытия базы; формулируй нейтрально: нужного в контексте нет (позже к ответу может подключиться другой источник).
- Не сочиняй цитаты, таблицы, места и цифры, которых нет во фрагментах.
...
- Если вопрос узкий («кто выиграл…», «какое место…», «сколько очков…») и во фрагментах есть именно тот факт, о котором спрашивают (имя, команда или цифра явно относятся к запросу) — начни message с этого факта (одно–два предложения достаточно).
- Если во фрагментах явно указан победитель или итог (например «Итог гонки», «место 1», P1) именно для того события/сезона, о котором спрашивают (это следует из текста фрагментов вместе с вопросом) — назови пилота и команду; не говори, что данных нет.
- Не выдавай только текст поискового запроса или список источников без содержательного предложения с фактом (или без честного отказа, если фактов по вопросу во фрагментах нет).
...
Оцени, достаточно ли контекста, чтобы ответить на вопрос ТОЛЬКО на основе этих фрагментов, без догадок и без общих знаний.
sufficient=true только если во фрагментах есть прямые сведения, относящиеся к запросу (конкретный итог, участник, сезон/гонка из вопроса и т.п.). Если фрагменты про другие годы/события или слишком общие, чтобы ответить на вопрос — sufficient=false. Не опирайся на предполагаемую «полноту» базы по годам.
...
Реши, отвечает ли предлагаемый текст на вопрос по существу.

Требования:
- Для вопросов «кто выиграл / победитель / кто первый» достаточно краткого ответа с именем (и при наличии команды или года) — это accept=true.
- Для вопросов про таблицу чемпионата или итоги сезона достаточно ответа со списком или ссылкой на факты из текста ответа — accept=true, если есть конкретика, а не отписка.
- Ответ не должен быть только повтором вопроса, только поисковой строкой или только перечнем URL без фактов.
- Если текст явно не отвечает на вопрос или утверждает отсутствие данных при том, что в формулировке есть явные факты (имена, команды), оцени строже только если эти факты противоречат вопросу.
...
def _evidence_block_for_llm(item: EvidenceItem) -> str:
    body = (item.context_for_llm or "").strip() or item.snippet
    if len(body) > 14_000:
        body = body[:14_000] + "…"
    return body
